...
const Editlist = () => {
    ...
    useEffect(() => {
      ...
      const currentVal = model[iD];
      setList({shortName: currentVal.shortName, longName: currentVal.longName})
}, [])

 const onSubmit = async e => {
  e.preventDefault();
  const model = getModel();
  model[id].longName = list.longName;
  model[id].shortName = list.shortName;
  setModel(model);
  ...
  headers: {
    ...
    return (
        ...
        <h2 className="text-center mb-4 border">Edit Entries</h2>
